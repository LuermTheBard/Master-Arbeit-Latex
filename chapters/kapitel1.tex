...
The uncertainty of the measured lag is estimated using a Monte Carlo approach combining flux randomization (FR) and random subset selection (RSS) \parencite{Peterson_1998b, peterson2004}. In the FR step, each flux value is randomly perturbed according to its measurement uncertainty. In the RSS step, a set of $N$ data points is randomly drawn with replacement, but redundant selections are discarded, resulting in a new light curve with $M \leq N$ points. For each realization, the ICCF analysis is repeated, yielding a distribution of centroid lags. The uncertainties are estimated from the distribution of centroid lags obtained through the simulations\parencite{peterson2004}.The 16th and 84th percentiles of this distribution are adopted as the bounds of the $1\sigma$ confidence interval for the error estimation \parencite{Peterson_1998b}.
\subsection{Black-Hole Mass}
\label{subsec:BHM}

The reverberation mapping method can further be used to calculate the mass of the central SMBH. Based on the assumption that the dynamics of the BLR is dominated by the central SMBH it is possible to calculate its mass by using the viral mass \parencite{peterson2004}. Following  \cite{peterson2004}, the centroid lag of broad emission lines, it can be determined together with the velocity dispersion $\Delta V$ of the BLR

\begin{equation}
	M_{\mathrm{vir}} = \frac{R_{\mathrm{BLR}}\,\Delta V^2}{G},
\end{equation}

and following that the mass of the black hole:

\begin{equation}
	M_\mathrm{BH}= f \cdot M_{\mathrm{vir}}.
\end{equation}
Here, $G$ describes the gravitational constant and $f$ a scale factor which accounts for the unknown geometry, kinematics and inclination of the BLR \parencite{peterson2004}. The velocity dispersion of the BLR ccan be parametized by the line widths of the broad emission lines \parencite{peterson2004}. The scale factor $f$ can be calibrated by aligning reverberation-based masses with the $M_{\mathrm{BH}}-\sigma_*$ relation observed in not-active galaxies, with $\sigma_*$ as the stellar velocity dispersion of the galactic bulge \parencite{onken2004}. Several values for f has been calibrated based on different sample AGN naming $f=5.5$ \parencite{onken2004}, $f=4.31$ \parencite{gier2013a} and $f=3.6$ as examples. The way how the line width of the broad emission lines gets measured, influences the calibration of the scale factor \parencite{peterson2004}. There are too main methods to parameterize the line width $\sigma_\mathrm{line}$. The line dispersion, which is described in \cite{peterson2004}, and by using the FWHM, which will be used in this thesis. Following \cite{probst2025emission}, the scale factor $f = 1.8$ gets adopted, which gets adapted by taking the ratio $FWHM/\sigma_\mathrm{line} \approx 2$ from \cite{peterson2004} together with the scale factor found by \cite{graham2011expanded}
