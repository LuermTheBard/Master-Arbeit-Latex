...
The reverberation mapping methode can further be used to calculate the mass of the central SMBH. Having the centroid lag of broad emission lines, it can be determined together with the emission-line width $\Delta V$ of the BLR:

\begin{equation}
	M_\mathrm{BH}= \frac{fc\tau_\mathrm{centroid}\Delta V^2}{G}
\end{equation}
Here, $G$ describes the gravitational constant and $f$ a scale factor which accounts for the unknown geometry, kinematics and inclination of the BLR \parencite{peterson2004}. 





Following \cite{onken2004}, a mean value of $f$ can be calibrated by aligning reverberation-based masses with the $M_{\mathrm{BH}}-\sigma_*$ relation observed in not-active galaxies, giving $\left<f\right> = 5.5$. Here, $\sigma_*$ describes the stellar velocity dispersion of the galactic bulge.\\
