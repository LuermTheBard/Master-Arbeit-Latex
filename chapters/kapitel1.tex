...
The black hole mass is then obtained through

\begin{equation}
	M_{\mathrm{BH}} = f \cdot M_{\mathrm{vir}}
\end{equation}
Here, $G$ denotes the gravitational constant and $f$ is a scale factor that accounts for the unknown geometry, kinematics, and inclination of the BLR \parencite{peterson2004}. The velocity dispersion $\Delta V$ can be parameterized using the widths of the broad emission lines \parencite{peterson2004}.\\ 
The scale factor $f$ is calibrated by aligning reverberation-based black hole masses with the empirical $M_{\mathrm{BH}}-\sigma_*$ relation observed in inactive galaxies, where $\sigma_*$ denotes the stellar velocity dispersion of the galactic bulge \parencite{onken2004}. Different studies have reported values for $f$ based on various AGN samples, for example $f = 5.5$ \parencite{onken2004}, $f = 4.31$ \parencite{gier2013a}, and $f = 3.6$ \parencite{graham2011expanded}.\\
The calibration of the scale factor additionally depends on the measuring method for the line width of the broad emission lines \parencite{peterson2004}. Two commonly used parameterizations are the line dispersion $\sigma_{\mathrm{line}}$ (see \parencite{peterson2004}) and the FWHM. In this thesis, the FWHM is used to characterize the line width.\\
Following \cite{probst2025emission}, a scale factor of $f = 1.8$ is adopted. This value is adjusted using the relation $\mathrm{FWHM}/\sigma_{\mathrm{line}} \approx 2$ from \cite{peterson2004} together with the scale factor reported by \cite{graham2011expanded}.
