...
To measure the time lag between the continuum emission and the broad emission lines, the emission-line light curves are correlated with the photometric \textit{Swift} UVW2 light curve presented in \cite{mchardy2018x}. This is done by determining the cross-correlation function (CCF) and measuring its centroid of the to 20\% of the CCF peak, as described in Section \ref{subsec:rm_ccf}. This calculation is done with GECHO, which applies the interpolated cross-correlation function (ICCF) method introduced by \cite{gaskell_peterson1986}. For this procedure, the light curves are normalized by subtracting the respective mean value from each data point and, to increase the effective sampling and measure correlation between two unevenly sampled light curves, flux values between data points are linearly interpolated for both light curves.\\
The normalized light curves and the resulting CCFs are presented in Figures \ref{fig:ccfs_optical} and \ref{fig:ccfs_UV}. Additionally, the top panel of each figure displays the UVW2 light curve together with its auto-correlation function, while the bottom panel compares both lightcurves of the UVW2 and the UV continuum around $1150\,\AA$ and shows the corresponding CCF and time lag which has to be added for the final estimation of the characteristic BLR radii for each emission line. The resulting time lags towards the UVW2 continuum and the corresponding characteristic BLR radii are listed in Table \ref{tab:lags_UVW2}. The centroid uncertainties are estimated using the flux randomization/random subset selection (FR/RSS) method described by \cite{Peterson_1998b}, with 10000 iterations and a lag bin size of $0.5$ days. The resulting centroid distribution is displayed by the gray histogram in the above named figures.\\
...
The mean profiles of the Balmer lines share an overall similar shape, characterized by a steep blue wing and an asymmetric broader red wing. The latter is not visible in the mean profile of H$\gamma$, where the red wing is blended with [O\,\textsc{iii}] $\lambda4363$ emission.\\
The rms profiles of the Balmer lines share an overall similar asymmetric double-peaked shape, with the blue peak carrying higher flux (see Figure \ref{fig:rms_Balmer}). The two peaks are consistently located at approximately $500\,\mathrm{km\,s^{-1}}$ and $2000\,\mathrm{km\,s^{-1}}$ across all Balmer lines, resulting in a shift of the rms profiles towards positive velocities. While the profiles of H$\alpha$ and H$\beta$ show similarly steep slopes in both wings, those of H$\gamma$ and H$\delta$ display more extended blue wings, leading to a close resemblance between the rms profiles of  H$\alpha$ and H$\beta$, and between H$\gamma$ and H$\delta$.


\subsubsection{Ly$\alpha$}

The line profile of Ly$\alpha$ of both spectra exhibits absorption in the blue wing and is blended with N\,\textsc{v}$\,\lambda\lambda 1283,\,1242$ in the red wing. The rms profile showing a slightly broader line width than the mean profile and peak located at approximately $2000\,\mathrm{km\,s^{-1}}$ and therefore as wel shifted to positive velocities, like seen for the Balmer lines.
...
%	\small
%	\caption[Full Width Half Maxima]{Measured FWHM and of the mean and rms line profiles.}
%	\label{tab:line_widthj_FWHM}
%	\begin{tabular}{lc}
%		\hline
%		\hline
%		\textbf{Line} &  FWHM (rms)[km/s]   \\
%		\hline
%		\hline
%		H$\alpha$  &  $3100\pm 300$\\
%		H$\beta$ &  $3400 \pm 300$ \\
%		H$\gamma$ &  $3900 \pm 400$ \\
%		H$\delta$ &  $5900 \pm 2000$ \\
%		Ly$\alpha$   &  $4600\pm 500$\\
%		\hline
%		He\,\textsc{i}$\,\lambda5876$ &  $4000 \pm 400$  \\
%		He\,\textsc{ii}$\,\lambda1640$  & $6900 \pm 1500$\\
%		He\,\textsc{ii}$\,\lambda4686$&   $6000 \pm 400$ \\
		
%		\hline
		%N\,\textsc{v}$\,\lambda\lambda 1238,\,1242$ &    $3400 \pm 2000$\\
		%Si\,\textsc{iv}$\,\lambda\lambda 1393,\,1402$ &  $5200 \pm 500$&  $10000 \pm 1500$\\
%		C\textsc{iv}$\,\lambda\lambda 1548,\,1550$ & $ 8400 \pm 1000$\\
