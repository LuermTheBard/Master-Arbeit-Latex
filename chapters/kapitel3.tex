...
\textbf{N kann ich die rechte flake nehmen und mit 2 multipliieren, da die linke flanke mit lyalpha überlagert ist. Lyalpha höhere fehler annehmen. Si raus nehmen, da eine abschetzung wegen der duplet überlagerung schwierig ist. HeII rechte seite abschätzen und mal 2, nur wenn es mit der anderen He II linie übereinstimmt einigermaßen. CIV sieht gut aus.}
...
\end{equation}
As describes in Section \ref{subsec:BHM}, the line width used to describe the velocity dispersion $\Delta V^2$ of the broad emission line. For this analysis, the line width gets measured using the FWHM of the RMS line profile, to only measure the broad line component of the emission line. Using this measure a average scale factor of $f =1.8$ gets assumed \cite{probst2025emissionlinecontinuumreverberationmapping}. The resulting SMBH mass results, as well as the corresponding time lag and FWHM values of the broad emission lines are listed in Table \ref{tab:BH_mass}. 



But because NGC\,4593 shows a low-inclination $i \sim 11^\circ$ of the elliptic accretion disk, modelled in \cite{ochmann2024transient}, has to be taken into account. Using the scaling relation $f \sim \sin^{-2}(i)$, following \parencite{krolik_2001, Kollatschny_2003}, a value of 


\textbf{1.8 ist die untere grenze. Bei niedriger inclination kann der f faktor größer sein. inclination bei 11 durch das model. dadurch muss der f faktor um 5 höher sein etwa. Berechnen.}
