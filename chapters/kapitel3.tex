...
The widths of the emission line profiles in the mean and rms spectrum are measured by their Full Width Half Maximum (FWHM), were the height of the profiles is defined between their maximum and the zero-flux base line. This measure is conducted with GECHO for all emission lines with following exceptions. The mean profile of He\,\textsc{ii}$\,\lambda4685$ is partly blended with Fe\,\textsc{ii} emission in its blue wing, which is why the FWHM was estimated base on the width of the profiles red flank by doubling it. The same approach is used for the N\,\textsc{V}$\,\lambda\lambda 1238,\,1242$ profiles in both mean and rms spectra, since the blue wing is blended with the Ly$\alpha$ profile. \\
The mean profile of the NIR emisson line O\,\textsc{i}$\,\lambda 8446$ is blended with the Ca\,\textsc{ii} $\lambda8498$, $\lambda8542$, $\lambda8662$ triplet. In a earlier study, \cite{Ochmann_2025} was already able to decompose the line profile  O\,\textsc{i}$\,\lambda 8446$ based on a Muse campaign in 2019, which is why this was not attempted in this analysis. In addition to that, the RMS profile shows very strong noise due to the low signal to noise ratio. The resulting FWHM values are listed in Table \ref{tab:line_width_FWHM}.\\
