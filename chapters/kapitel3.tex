...
Looking in more detail into the shape of RMS profiles, the profiles of H$\alpha$ and H$\beta$ show the most similarity to each other, with both of their flanks following a similar shape. The same applies to the red flanks of H$\gamma$ and H$\delta$ with steep slope and a small peak at about $\sim 3000 \,\mathrm{km\,s^{-1}}$. Only the blue flank of H$\gamma$ and H$\delta$ is more extended and noisy as for H$\alpha$ and H$\beta$. This leads to a slightly higher FWHM value for H$\gamma$ with $\sim 3900 \pm 400\,\mathrm{km\,s^{-1}}$ compared to the values of H$\alpha$ and H$\beta$ with $\sim 3100 \pm 250\,\mathrm{km\,s^{-1}}$ and $\sim 3400 \pm 200\,\mathrm{km\,s^{-1}}$ and a much higher value of $\sim 5900 \pm 1000 \,\mathrm{km\,s^{-1}}$ for H$\delta$. Taking the similarities of the RMS profile shapes, it can be assumed that all four Balmer lines emerge under similar kinematic properties. \\
...
To estimate the time lag of the measured emission lines to the ionizing continuum, the time lag between the light curve of the emission line and the UVW2 light curve is estimated. This is done, by calculating their cross-correlation-function (CCF) and estimating their centroid of its top $20\%$, as described in section \ref{subsec:rm_ccf}. The CCF is calculated with the help of GECHO, by applying the interpolated cross-correlation function (ICCF) methode, introduced by \cite{gaskell_peterson1986}. The light curves of the emission line and the reference spectrum are getting normalized by subtracting their mean value from each data point. To acquire a higher sample size and to be able to derive correlation between two light curves with different sample size, flux values between each data points are linear interpolated for both light curves.
The normalized light curves and the resulting CCFs are displayed in Figure \ref{fig:ccfs_optical} and \ref{fig:ccfs_UV}. The top panel of each figure shows the   UVW2 light curve together with the corresponding auto-correlation function and the bottom panel a comparison of the UVW2 and UV continuum around $1150\,\AA$ together with the corresponding CCF. As mentioned in Section \ref{sec:lightcurves}, the most blue continuum is commonly used as a reference. Because the central wavelength of the UVW2 continuum is located at $\sim 1930\,\AA$, the time lag between the both named continua has to be added time lag estimation. The calculated time lags between each light curve are listed in Table \ref{tab:lags_UVW2}. The uncertainties of the centroids are estimated by applying the flux randomization/random subset selection (FR/RSS) methode by \cite{Peterson_1998b}  with 10000 iterations and a bin size of $0.5$ days (see Section \ref{subsec:rm_ccf}). The resulting centroid distribution is displayed by the gray histogram in the above named figures.\\
Overall, the CCFs of all measured emission lines show strong correlations with a pronounced maxima between $\sim 0.7$ -- $0.9$, except the CCF of O\,\textsc{i}\,8446 which shows two maxima of around $\sim 0.6$. The Helium\,\textsc{ii} lags the least behind the UVW2 light curve with an average lag of $\sim 0.5$ days within their uncertainties. He\,\textsc{ii}\,4685 shows here a higher correlation with $\sim 0.8$ than He\,\textsc{ii}\,1640 with $\sim 0.7$. The correlation of the other UV lines Ly$\alpha$, N\,\textsc{v}\,1238, Si\,\textsc{iv}\,1393 and C\,\textsc{iv}\,1548 all peaks at above $\sim 0.8$, with Ly$\alpha$ even showing a maximum of $\sim 0.9$, and show similar lags of $\sim 1$ day within their uncertainties. The Balmer lines lags at higher values, with H$\alpha$ and H$\beta$ lagging behind the UVW2 light curve with $3.2 \ensuremath{_{-0.6}^{+1.0}}$ and $2.3 \ensuremath{_{-0.3}^{+1.0}}$ days and H$\gamma$ and H$\delta$ with $1.7 \ensuremath{_{-0.4}^{+0.2}}$ and $1.7 \ensuremath{_{-0.4}^{+0.4}}$ days respectively. H$\beta$ shows the highest correlation of the Balmer lines with over $\sim 0.85$, while H$\alpha$ shows the lowest with about $\sim 0.8$. Compared to H$\gamma$ and H$\delta$, they show also a broader peak and therefore centroid distribution in higher uncertainties. He\,\textsc{i}\,5875 and O\,\textsc{i}\,8446 show the lowest correlation with the UVW2 light curve with $\sim 0.7$ and $\sim 0.6$ respectively. He\,\textsc{i}\,5875 shows also a similar lag of $3.3 \ensuremath{_{-0.4}^{+0.7}}$ days compared to H$\alpha$ and H$\beta$ within uncertainties, while O\,\textsc{i}\,8446 lags the longest with $5.0 \ensuremath{_{-1.7}^{+2.1}}$ days behind the UVW2 light curve, with high uncertainties due to the lower correlation of their CCF.
...
\begin{table}[!htb]
	...
	\begin{tabular}{l c | l c}
		...
		H$\delta$             & $1.7 \ensuremath{_{-0.4}^{+0.4}}$ & Si\,\textsc{iv}\,1393  & $1.2 \ensuremath{_{-0.4}^{+0.1}}$ \\
		Ly$\alpha$            & $1.0 \ensuremath{_{-0.2}^{+0.4}}$ & N\,\textsc{v}\,1238    & $1.0 \ensuremath{_{-0.4}^{+0.2}}$ \\
		O\,\textsc{i}\,8446   & $5.0 \ensuremath{_{-1.7}^{+2.1}}$ & C\,\textsc{iv}\,1548   & $0.8 \ensuremath{_{-0.1}^{+0.5}}$ \\
		\hline\hline
	\end{tabular}
\end{table}
...
\begin{table}[!ht]
	...
	\begin{tabular}{l c c c}
		...
		N\,\textsc{v}\,$\,\lambda1238$ & $1.0 \ensuremath{_{-0.4}^{+0.2}}$ & $3383 \pm 1000$ & $0.4 \ensuremath{_{-0.3}^{+0.4}}$ \\
