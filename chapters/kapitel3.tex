...
Apart of the variability, it is possible to investigate the kinematics of the broad line region, by extracting the line profiles of given broad emission lines and investigate their shape and line width. Following the same procedure as for the lightcurve extraction, a linear underlying continuum gets interpolated using a chosen red and blue pseudo-continua for each line in either the AVG and RMS spectrum. The selected boundaries of the pseudo-continua can be found in Table \ref{tab:line_profiles_pseudo}. This interpolated continuum gets subtracted to define a base line at zero flux for every line profile for better comparison. Subsequently, the line gets converted into the velocity space using 

\begin{equation}
	v_i = \frac{(\lambda_i - \lambda_\mathrm{central})}{\lambda_\mathrm{central} \cdot c}.
\end{equation}\\
Here donates $\lambda_i$ the wavelength values, $\lambda_\mathrm{cetral}$ the central wavelength of the emission line and $c$ the speed of light. The flux gets normalized to the maximum of the emission lines. Figure \ref{fig:line_profiles} shows an overview of the extracted and normalized line profiles of H$\alpha$, H$\beta$, H$\gamma$, OI$\,\lambda 8446$, HeI$\,\lambda5875$ and  HeII$\,\lambda4685$ in the AVG spectrum as well as in the RMS spectrum. \\
The Balmer Lines, as well as the Helium lines, show well-defined profiles in both AVG and RMS, qualifying them for further investigation of their kinematics. In case of the  OI$\,\lambda 8446$ emission line, the line profile overlaps with the Ca\,\textsc{ii}$\,\lambda8498\,\lambda8542\,\lambda8662$ triplet in the AVG spectrum. The RMS is very noisy, which makes a further investigation of the kinematics of this emission line not expedient for this analysis. 
\begin{figure}[!ht]
	\centering
...
A comparison of the Balmer-line profiles H$\alpha$, H$\beta$, and H$\gamma$ is shown in Figure \ref{fig:AVG_RMS_Balmer}. With the exception of the AVG profile of H$\gamma$, which is affected by the narrow [O III] $\lambda 4363$ line overlapping the H$\gamma$ profile, the lines exhibit a very similar asymmetric shape their AVG profiles. While all three lines show a similar steep blue wing, an increased broadening of the red wing is clearly noticeable in H$\alpha$ and H$\beta$ starting at $\approx 1500 \, \mathrm{km/s}$. It has to be noted that the both line profiles are influenced by narrow components, which results in slightly different line widths. This components are hardly distinguishable, which is why it was not attempt to correct the profiles, by subtracting the narrow components from the profiles.\\
This problem does not occur in the RMS line profiles (see. Chapter \ref{sec:spectra}). Looking in the second row of Figure \ref{fig:AVG_RMS_Balmer}, all three lines show an asymmetric double-peaked shape, with higher flux variation in the blue peak. The profile of H$\alpha$ and H$\beta$ are nearly identical, while H$\gamma$'s blue wing is more spread and noisy at higher velocities.\\
The close similarities of all three Balmer-lines indicates that they originate all from the same region.
